...
import { Wrench, Briefcase, MapPin, IndianRupee, Image, AlertCircle, ArrowRight, Clock } from 'lucide-react';

const BecomeProvider = () => {
  const { becomeProvider, user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await becomeProvider(payload);
      navigate('/provider/dashboard');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to submit provider registration');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl w-full bg-white rounded-3xl border border-slate-200/80 p-8 shadow-xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-extrabold text-slate-900">Become a Service Provider</h2>
          <p className="text-xs text-slate-500">
            Set up your professional profile to start receiving service requests
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-xs focus:border-indigo-600 focus:outline-hidden"
                  >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs focus:border-indigo-600 focus:outline-hidden"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">About Your Service (Description)</label>
              <textarea
                name="description"
                rows="3"
                ...
                className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs focus:border-indigo-600 focus:outline-hidden"
              />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-4 bg-white p-3 rounded-2xl border border-slate-200">
                <div className="relative shrink-0">
                  <img
                    src={formData.profileImage && formData.profileImage.trim() !== '' ? formData.profileImage : `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'Provider')}&background=6366f1&color=fff`}
                    alt="Preview"
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-2xs focus:bg-white focus:border-indigo-600 focus:outline-hidden"
                  />
                </div>
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Initial Availability</label>
              <div className="relative">
          ...
          >
            {submitting ? 'Registering as Provider...' : 'Submit & Register as Provider'}
