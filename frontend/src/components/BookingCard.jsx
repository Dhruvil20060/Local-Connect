...
import { Calendar, Clock, MapPin, CheckCircle2, XCircle, PlayCircle, Star, AlertCircle, FileText } from 'lucide-react';

const BookingCard = ({ booking, userRole, onUpdateStatus, onOpenReviewModal }) => {
  ...
  const otherPersonName = userRole === 'customer' ? providerId?.name : customerId?.name;
  const otherPersonPhone = userRole === 'customer' ? providerId?.phone : customerId?.phone;
  ...
  const getStatusBadge = (st) => {
    switch (st) {
      ...
      case 'Completed':
        return <span className="px-3 py-1 text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" /> Completed</span>;
      case 'Rejected':
      ...
      default:
        return <span className="px-3 py-1 text-xs font-bold text-slate-700 bg-slate-50 border rounded-full">{st}</span>;
    }
  };
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs hover:shadow-md transition-all flex flex-col justify-between">
      <div>
        ...
        <div className="py-4 space-y-3 text-xs text-slate-600">
          ...
          {/* Contact Details visible when accepted / in progress / completed */}
          {['Accepted', 'In Progress', 'Completed'].includes(status) && otherPersonPhone && (
      ...
      <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {/* Provider Actions */}
          {userRole === 'provider' && status === 'Requested' && (
            <>
              <button
                onClick={() => onUpdateStatus(_id, 'Accepted')}
                className="px-3.5 py-1.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-2xs"
              >
                Accept Job
              </button>
              <button
                onClick={() => onUpdateStatus(_id, 'Rejected')}
                className="px-3.5 py-1.5 text-xs font-bold text-rose-700 bg-rose-50 hover:bg-rose-100 rounded-xl transition-all border border-rose-200"
              >
                Reject
              </button>
            </>
          ...
          {userRole === 'provider' && status === 'Accepted' && (
            <button
              onClick={() => onUpdateStatus(_id, 'In Progress')}
              className="px-3.5 py-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-all shadow-2xs"
            >
              Start Job
            </button>
          )}

          {userRole === 'provider' && status === 'In Progress' && (
            <button
          ...
          {/* Customer Actions */}
          {userRole === 'customer' && ['Requested', 'Accepted'].includes(status) && (
            <button
              onClick={() => onUpdateStatus(_id, 'Cancelled')}
              className="px-3.5 py-1.5 text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all"
            >
              Cancel Booking
            </button>
          )}

          {userRole === 'customer' && status === 'Completed' && (
            <button
              onClick={() => onOpenReviewModal(booking)}
              className="px-3.5 py-1.5 text-xs font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 rounded-xl transition-all border border-amber-200 flex items-center gap-1.5"
            >
